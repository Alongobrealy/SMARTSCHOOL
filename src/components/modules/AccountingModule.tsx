    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  shadow-sm transition-colors duration-200 text-center md:text-left">
        <div>
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-slate-100  mt-1">États Financiers & Grand Livre de Trésorerie</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  p-5 rounded-2xl shadow-sm flex flex-col justify-between transition-colors duration-200">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-blue-600  uppercase tracking-wider">Total Recettes Encaissées</span>
            <div className="w-8 h-8 rounded-lg bg-blue-50  text-blue-600  border border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  flex items-center justify-center">
              <ArrowDownLeft className="w-4 h-4" />
            </div>
          </div>
          <p className="text-2xl sm:text-3xl font-bold text-blue-600  mt-3">+{totalRevenues.toLocaleString()} FCFA</p>
        ...
        <div className={`p-5 rounded-lg border shadow-sm flex flex-col justify-between transition-colors duration-200 ${
          netSolde >= 0 
            ? 'bg-blue-50/70  border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60 ' 
            : 'bg-rose-50/70  border-rose-200 '
        }`}>
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-blue-600  uppercase tracking-wider">Solde Net Disponible</span>
            <ShieldCheck className="w-5 h-5 text-blue-600 " />
          </div>
          <p className="text-2xl sm:text-3xl font-bold text-blue-600  mt-3">{netSolde.toLocaleString()} FCFA</p>
          <span className="text-[11px] text-blue-600  font-medium mt-1">Disponibilité Caisse & Comptes Bancaires</span>
        </div>
      </div>

      {/* Mode Tabs */}
      <div className="flex items-center gap-2 bg-white dark:bg-slate-900 p-2 rounded-2xl border border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  shadow-sm text-xs transition-colors duration-200">
        <button
      ...
      {activeTab === 'analytics' && (
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  shadow-sm p-5 sm:p-6 transition-colors duration-200">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  pb-4 mb-4">
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              ...
              <div className="flex flex-col gap-2">
                {expenseCategoryData.map((cat) => (
                  <div 
                    key={cat.name}
                    className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50  border border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  text-xs"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  rounded-2xl shadow-sm overflow-hidden transition-colors duration-200">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  flex items-center justify-between">
              ...
              <span className="text-xs text-blue-600  font-bold">+{totalRevenues.toLocaleString()} FCFA</span>
            </div>

            <div className="divide-y divide-slate-100 ">
              {payments.slice(0, 5).map((p) => (
                <div key={p.id} className="p-4 sm:px-6 flex items-center justify-between text-xs hover:bg-slate-50 dark:bg-slate-800/50/70 transition-colors">
                  <div>
                  ...
                  <div className="text-right">
                    <span className="font-mono font-bold text-blue-600  text-sm">+{p.montant.toLocaleString()} FCFA</span>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400  font-medium">{p.modePaiement}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  rounded-2xl shadow-sm overflow-hidden transition-colors duration-200">
            ...
            <div className="divide-y divide-slate-100 ">
              {expenses.map((e) => (
                <div key={e.id} className="p-4 sm:px-6 flex items-center justify-between text-xs hover:bg-slate-50 dark:bg-slate-800/50/70 transition-colors">
                  <div>
                    <p className="font-bold text-slate-800 dark:text-slate-100 ">{e.titre}</p>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-slate-500 dark:text-slate-400  font-medium">{e.date}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-white dark:bg-slate-900 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100  w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  shadow-2xl p-6 flex flex-col gap-4">
            ...
            <form onSubmit={handleSaveExpense} className="flex flex-col gap-3.5 text-xs">
              <div>
                <label className="text-slate-800 dark:text-slate-100  font-semibold mb-1 block">Libellé de la dépense :</label>
                <input
                  type="text"
                  value={expenseTitle}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-800 dark:text-slate-100  font-semibold mb-1 block">Catégorie :</label>
                  <select
                ...
                <div>
                  <label className="text-slate-800 dark:text-slate-100  font-semibold mb-1 block">Montant (FCFA) :</label>
                  <input
                    type="number"
                    min="500"
                    step="500"
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800/50  border border-slate-200 dark:border-slate-700/60 dark:border-slate-700/60  text-rose-600  rounded-xl px-3 py-2 font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    required
                  />
                </div>
              </div>
              ...
              <div>
                <label className="text-slate-800 dark:text-slate-100  font-semibold mb-1 block">Bénéficiaire / Prestataire :</label>
                <input
                  type="text"
