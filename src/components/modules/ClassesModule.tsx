    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm transition-colors duration-200 text-center md:text-left">
        <div>
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white mt-1">Gestion des Classes & Horaires de Cours</h2>
      ...
      {/* Class Selector Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm text-xs transition-colors duration-200">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full">
          <span className="text-slate-600 dark:text-slate-300 font-semibold text-center sm:text-left">Sélectionner la classe :</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {days.map(day => {
          ...
          return (
            <div
              key={day}
              className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden flex flex-col transition-colors duration-200"
            >
              <div className="bg-slate-50 dark:bg-slate-800/70 px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <span className="font-bold text-slate-800 dark:text-white text-sm">{day}</span>
                <span className="text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/60 px-2 py-0.5 rounded-md border border-indigo-100 dark:border-indigo-800/60">
              ...
              <div className="p-3 flex-1 flex flex-col gap-2.5">
                {dayCourses.length > 0 ? (
                  dayCourses.map(course => (
                    <div
                      key={course.id}
                      className="bg-slate-50/70 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-500 p-3 rounded-xl flex flex-col gap-1.5 transition-colors"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-slate-900 dark:text-white text-xs">{course.matiere}</span>
                        <span className="text-[10px] font-mono text-indigo-700 dark:text-indigo-300 font-semibold bg-indigo-50 dark:bg-indigo-950/70 px-1.5 py-0.5 rounded border border-indigo-100 dark:border-indigo-800/60">
                          {course.heureDebut} - {course.heureFin}
                        </span>
                      </div>

                      <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 pt-1 border-t border-slate-200/50 dark:border-slate-700">
                  ...
                  <div className="h-24 flex items-center justify-center text-xs text-slate-400 dark:text-slate-500 italic">
          ...
          <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl p-6 flex flex-col gap-4">
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-3.5 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-700 dark:text-slate-300 font-semibold mb-1 block">Jour :</label>
                  <select
                    value={formJour}
                    onChange={(e) => setFormJour(e.target.value as any)}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer"
                  >
                    {days.map(d => <option key={d} value={d}>{d}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-slate-700 dark:text-slate-300 font-semibold mb-1 block">Salle de cours :</label>
                  <input
                    type="text"
                    value={formSalle}
                    onChange={(e) => setFormSalle(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    required
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-700 dark:text-slate-300 font-semibold mb-1 block">Heure de début :</label>
                  <input
                    type="time"
                ...
                <div>
                  <label className="text-slate-700 dark:text-slate-300 font-semibold mb-1 block">Heure de fin :</label>
                  <input
                    type="time"
                    value={formHeureFin}
              ...
              <div>
                <label className="text-slate-700 dark:text-slate-300 font-semibold mb-1 block">Enseignant titulaire :</label>
                <input
                  type="text"
