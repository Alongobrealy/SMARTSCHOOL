...
export const HRModule: React.FC<HRModuleProps> = ({ 
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Header */}
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-200 text-center md:text-left">
        <div>
        ...
        {canEdit && (
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2 w-full md:w-auto">
            <button
              ...
              className="w-full sm:w-auto px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-sm flex items-center justify-center gap-1.5 transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="w-full sm:w-auto px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow-sm flex items-center justify-center gap-1.5 transition-all cursor-pointer"
            >
              <Plus className="w-4 h-4" />
              + Personnel Admin
            </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-sm flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800/60 flex items-center justify-center font-bold">
            <Users className="w-6 h-6" />
          </div>
          <div>
            <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Corps Professoral</span>
            <p className="text-xl font-black text-slate-900 dark:text-white">{teachers.length} Enseignant(s)</p>
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-sm flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-xl bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800/60 flex items-center justify-center font-bold">
            <UserCheck className="w-6 h-6" />
          </div>
          <div>
            <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Personnel Administratif</span>
            <p className="text-xl font-black text-slate-900 dark:text-white">{staff.length} Agent(s)</p>
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-sm flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-xl bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-400 border border-purple-100 dark:border-purple-800/60 flex items-center justify-center font-bold">
            <DollarSign className="w-6 h-6" />
          </div>
          <div>
            <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Masse Salariale Totale</span>
            <p className="text-xl font-black text-emerald-600 dark:text-emerald-400">{grandTotalPayroll.toLocaleString()} FCFA / mois</p>
          </div>
        </div>
      </div>
      ...
      {activeTab === 'teachers' && (
        <div>
          {teachers.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 border-2 border-dashed border-slate-200 dark:border-slate-800 text-center">
              <Users className="w-12 h-12 text-slate-400 mx-auto mb-3" />
              <h3 className="font-bold text-sm text-slate-900 dark:text-white">Aucun enseignant enregistré</h3>
              ...
              {canEdit && (
                <button
                  onClick={handleOpenAddTeacher}
                  className="mt-4 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold inline-flex items-center gap-1.5 cursor-pointer shadow-sm"
                >
                  <Plus className="w-4 h-4" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {teachers.map((t) => (
                <div
                  key={t.id}
                  className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm flex flex-col justify-between gap-4"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-100 dark:border-indigo-800/60 text-indigo-700 dark:text-indigo-300 flex items-center justify-center font-bold text-base">
                        {t.photoUrl ? (
                          <img src={t.photoUrl} alt={t.nom} className="w-full h-full object-cover rounded-2xl" />
                        ) : (
                          <span>{t.nom[0]}{t.prenom[0]}</span>
                        )}
                      </div>
                      <div>
                        <h4 className="font-extrabold text-slate-900 dark:text-white text-sm uppercase">{t.nom} <span className="capitalize text-slate-700 dark:text-slate-300 font-bold">{t.prenom}</span></h4>
                        <p className="text-xs text-indigo-600 dark:text-indigo-400 font-semibold">{t.specialite}</p>
                        <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500">{t.matricule}</span>
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-100 dark:border-slate-800 flex flex-col gap-1.5 text-xs">
                    <div className="flex items-center justify-between text-slate-600 dark:text-slate-300">
                      <span className="text-slate-500 dark:text-slate-400">Matières :</span>
                      <span className="font-semibold text-slate-900 dark:text-white">{t.matieres.join(', ')}</span>
                    </div>
                    <div className="flex items-center justify-between text-slate-600 dark:text-slate-300">
                      <span className="text-slate-500 dark:text-slate-400">Classes :</span>
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{t.classes.join(', ')}</span>
                    </div>
                    <div className="flex items-center justify-between text-slate-600 dark:text-slate-300">
                      <span className="text-slate-500 dark:text-slate-400">Salaire Mensuel :</span>
                      <span className="font-bold text-emerald-600 dark:text-emerald-400">{t.salaireMensuel.toLocaleString()} FCFA</span>
                    </div>
                  </div>

                  <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
                    <span className="font-mono text-[11px] text-slate-500">{t.telephone}</span>

                    <div className="flex items-center gap-1.5">
                      {/* Badge d'accès */}
                      <button
                        onClick={() => setSelectedPersonForBadge({ person: t, type: 'teacher' })}
                        className="px-2.5 py-1.5 bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-600 text-indigo-700 dark:text-indigo-300 hover:text-white rounded-xl font-bold text-xs border border-indigo-200 dark:border-indigo-800 flex items-center gap-1 transition-all cursor-pointer"
                        title="Badge d'Accès"
                      >
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-600 text-emerald-700 dark:text-emerald-300 hover:text-white rounded-xl font-bold text-xs border border-emerald-200 dark:border-emerald-800 flex items-center gap-1 transition-all cursor-pointer"
                        title="Fiche de Paie"
                      >
        ...
        <div>
          {staff.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 border-2 border-dashed border-slate-200 dark:border-slate-800 text-center">
              ...
              <h3 className="font-bold text-sm text-slate-900 dark:text-white">Aucun personnel administratif enregistré</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">
              ...
              {canEdit && (
                <button
                  onClick={handleOpenAddStaff}
                  className="mt-4 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold inline-flex items-center gap-1.5 cursor-pointer shadow-sm"
                >
                  <Plus className="w-4 h-4" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {staff.map((s) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-100 dark:border-emerald-800/60 text-emerald-700 dark:text-emerald-300 flex items-center justify-center font-bold text-base">
                        {s.photoUrl ? (
                          <img src={s.photoUrl} alt={s.nom} className="w-full h-full object-cover rounded-2xl" />
                        ) : (
                          <span>{s.nom[0]}{s.prenom[0]}</span>
                        )}
                      </div>
                      <div>
                        <h4 className="font-extrabold text-slate-900 dark:text-white text-sm uppercase">{s.nom} <span className="capitalize text-slate-700 dark:text-slate-300 font-bold">{s.prenom}</span></h4>
                        <p className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold">{s.roleFonction}</p>
                        <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500">{s.matricule}</span>
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-100 dark:border-slate-800 flex flex-col gap-1.5 text-xs">
                    <div className="flex items-center justify-between text-slate-600 dark:text-slate-300">
                      ...
                      <span className="font-semibold text-slate-900 dark:text-white">{s.datePriseService}</span>
                    </div>
                    <div className="flex items-center justify-between text-slate-600 dark:text-slate-300">
                      <span className="text-slate-500 dark:text-slate-400">Statut :</span>
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{s.statut}</span>
                    </div>
                    <div className="flex items-center justify-between text-slate-600 dark:text-slate-300">
                      <span className="text-slate-500 dark:text-slate-400">Salaire :</span>
                      <span className="font-bold text-emerald-600 dark:text-emerald-400">{s.salaireMensuel.toLocaleString()} FCFA</span>
                    </div>
                  </div>

                  <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
                    <span className="font-mono text-[11px] text-slate-500">{s.telephone}</span>
      ...
      {selectedTeacherForSlip && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 w-full max-w-lg rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl p-6 flex flex-col gap-5">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3 flex items-start justify-between">
              <div>
                <h3 className="font-black text-indigo-950 dark:text-white text-lg">BULLETIN DE PAIE MENSUEL</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {schoolConfig.name || 'Établissement Scolaire'} • {schoolConfig.city || 'Brazzaville'}
                </p>
              </div>
              <span className="bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 text-xs font-bold px-2 py-1 rounded-md">
                VIREMENT VALIDÉ
              </span>
            </div>

            <div className="bg-slate-50 dark:bg-slate-800/70 p-4 rounded-xl border border-slate-200/80 dark:border-slate-700 grid grid-cols-2 gap-3 text-xs">
              <div>
                <span className="text-slate-500 dark:text-slate-400">Nom du Bénéficiaire :</span>
                <p className="font-bold text-slate-900 dark:text-white">{selectedTeacherForSlip.prenom} {selectedTeacherForSlip.nom}</p>
              </div>
              <div>
                <span className="text-slate-500 dark:text-slate-400">Matricule :</span>
                <p className="font-mono font-bold text-slate-900 dark:text-white">{selectedTeacherForSlip.matricule}</p>
              </div>
              <div>
                <span className="text-slate-500 dark:text-slate-400">Statut :</span>
                <p className="font-semibold text-slate-900 dark:text-white">{selectedTeacherForSlip.statut}</p>
              </div>
              <div>
                <span className="text-slate-500 dark:text-slate-400">Heures Prestées :</span>
                <p className="font-bold text-slate-900 dark:text-white">{selectedTeacherForSlip.heuresEffectuees} heures</p>
              </div>
            </div>

            <div className="border border-slate-200/80 dark:border-slate-700 rounded-xl overflow-hidden text-xs">
              <div className="bg-slate-50 dark:bg-slate-800 p-2.5 font-bold flex justify-between text-slate-700 dark:text-slate-200">
                <span>Rubrique de Rémunération</span>
                <span>Montant Net</span>
              </div>
              <div className="p-3 divide-y divide-slate-100 dark:divide-slate-800 flex flex-col gap-2">
                <div className="flex justify-between py-1">
                  <span className="text-slate-600 dark:text-slate-300">Salaire de base</span>
                  <span className="font-semibold text-slate-900 dark:text-white">{selectedTeacherForSlip.salaireMensuel.toLocaleString()} FCFA</span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-slate-600 dark:text-slate-300">Prime d'exercice & transport</span>
                  <span className="font-semibold text-emerald-600 dark:text-emerald-400">+ 25 000 FCFA</span>
                </div>
                <div className="flex justify-between py-1 text-slate-500 dark:text-slate-400">
                  <span>Cotisation CNSS Congo (4%)</span>
                  <span className="font-semibold text-rose-600 dark:text-rose-400">- {(selectedTeacherForSlip.salaireMensuel * 0.04).toLocaleString()} FCFA</span>
                </div>
                <div className="flex justify-between pt-2 font-bold text-sm text-indigo-950 dark:text-white border-t border-slate-200 dark:border-slate-700">
      ...
      {showTeacherModal && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 my-6">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              <h3 className="font-bold text-base">
                {editingTeacher ? 'Modifier l\'Enseignant' : 'Ajouter un Nouvel Enseignant'}
              </h3>
              <button onClick={() => setShowTeacherModal(false)} className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 font-semibold font-mono"
                  />
                </div>
                <div>
              ...
              <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
                  >
                    Annuler
                  </button>
                  <button
                    type="submit"
                    className="px-4 py-2 rounded-xl font-bold bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm cursor-pointer hover:scale-105 active:scale-95 transition-all"
                  >
        ...
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 my-6">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              ...
              <button onClick={() => setShowStaffModal(false)} className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveStaff} className="space-y-3 text-xs">
              ...
              <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 rounded-xl font-bold bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm cursor-pointer hover:scale-105 active:scale-95 transition-all"
                  >
