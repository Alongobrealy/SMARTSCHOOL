    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white  p-4 sm:p-6 rounded-lg border border-[#E4E6EB]  shadow-sm transition-colors duration-200 text-center md:text-left">
        <div>
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-[#050505]  mt-1">Portail Web & Proclamation des Résultats</h2>
      ...
      {/* Tabs */}
      <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-2 bg-white  p-2.5 rounded-lg border border-[#E4E6EB]  shadow-sm text-xs w-full sm:w-fit transition-colors duration-200">
        <button
          onClick={() => setActiveTab('actualites')}
          className={`w-full sm:w-auto px-4 py-2 rounded-xl font-bold transition-all cursor-pointer text-center ${
            activeTab === 'actualites'
              ? 'bg-[#1877F2] text-white shadow-sm'
              : 'bg-transparent text-[#65676B]  hover:bg-[#F0F2F5] hover:text-[#050505]'
          }`}
        >
      ...
      {activeTab === 'actualites' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {announcements.map((anc) => (
            <div
              key={anc.id}
              className="bg-white  border border-[#E4E6EB]  hover:border-[#E4E6EB][#E4E6EB] p-5 rounded-lg shadow-sm transition-all flex flex-col justify-between gap-4"
            >
              <div className="flex flex-col gap-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-[#65676B]  font-medium">{anc.datePublication}</span>
                </div>

                <h3 className="font-bold text-[#050505]  text-base hover:text-[#1877F2] transition-colors">
                  {anc.titre}
                </h3>
              ...
              <div className="pt-3 border-t border-[#E4E6EB]  flex items-center justify-between text-xs text-[#65676B] ">
                <span>Par : <strong className="text-[#050505] ">{anc.auteur}</strong></span>
                <span className="bg-[#F0F2F5]  text-[#65676B]  px-2 py-0.5 rounded text-[10px] font-medium border border-[#E4E6EB] ">Cible: {anc.cible}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-6 max-w-2xl mx-auto w-full">
          <div className="bg-white  border border-[#E4E6EB]  p-8 rounded-lg shadow-sm flex flex-col gap-4 text-center transition-colors duration-200">
            <div className="w-16 h-16 rounded-lg bg-[#E7F3FF]  text-[#1877F2]  border border-[#E4E6EB]  flex items-center justify-center mx-auto">
              <Award className="w-8 h-8" />
            </div>
            <div>
              <h3 className="text-xl font-bold text-[#050505] ">Guichet Numérique de Proclamation</h3>
            ...
            <form onSubmit={handleSearchProclamation} className="flex gap-2 mt-2">
              <input
                ...
                className="flex-1 bg-[#F0F2F5]  border border-[#E4E6EB]  text-[#050505]  rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white font-mono"
              />
              <button
                type="submit"
                className="px-5 py-2.5 bg-[#1877F2] hover:bg-[#1877F2] text-white font-bold rounded-xl text-xs flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
              >
          ...
          {/* Search Result */}
          {searchedStudent && (
            <div className="bg-white  border border-[#E4E6EB]  p-6 rounded-lg shadow-md flex flex-col gap-4 transition-colors duration-200">
              <div className="flex items-start justify-between border-b border-[#E4E6EB]  pb-3">
                <div>
                  <span className="text-[10px] uppercase font-bold text-[#1877F2]  tracking-wider">Résultat Proclamé</span>
                  <h4 className="text-lg font-bold text-[#050505] ">{searchedStudent.nom} {searchedStudent.prenom}</h4>
                  <p className="text-xs text-[#65676B] ">{searchedStudent.classe} • {searchedStudent.matricule}</p>
                </div>
                <div className="bg-[#E7F3FF]  text-[#1877F2]  border border-[#E4E6EB]  px-3 py-1 rounded-xl text-xs font-bold">
                  ADMIS(E) AVEC SUCCÈS
                </div>
              </div>
              ...
              <div className="overflow-x-auto text-xs">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[#F0F2F5]  text-[#65676B]  font-semibold border-b border-[#E4E6EB] ">
                  ...
                  <tbody className="divide-y divide-slate-100 ">
                    {grades.filter(g => g.studentId === searchedStudent.id || g.studentName.includes(searchedStudent.nom)).map(g => (
                      <tr key={g.id} className="hover:bg-[#F0F2F5]/60">
                        <td className="p-3 font-medium text-[#050505] ">{g.matiere}</td>
                        <td className="p-3 text-center text-[#65676B] ">{g.noteDevoir}/20</td>
                        <td className="p-3 text-center text-[#65676B] ">{g.noteExamen}/20</td>
                        <td className="p-3 text-center font-bold text-[#1877F2] ">
      ...
      <div className="bg-white  border border-[#E4E6EB]  p-6 rounded-lg shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 mt-2 transition-colors duration-200">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#1877F2] text-white flex items-center justify-center font-bold text-sm shadow-sm">
            EC
          </div>
          <div>
            <h4 className="font-bold text-[#050505]  text-sm">EDU-CONGO • Portail Numérique Institutionnel</h4>
            <p className="text-xs text-[#65676B] ">République du Congo • Brazzaville & Pointe-Noire</p>
          </div>
        </div>

        <div className="flex flex-wrap items-center gap-4 text-xs font-mono text-[#65676B] ">
          <span className="text-[#1877F2]  font-bold">WhatsApp : +242 06 895 83 77</span>
          <span className="text-[#1877F2]  font-bold">Appel : +242 06 169 35 98</span>
          <span className="text-[#050505] ">steph.alongo@gmail.com</span>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-white backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white  text-[#050505]  w-full max-w-md rounded-lg border border-[#E4E6EB]  shadow-2xl p-6 flex flex-col gap-4">
            ...
            <form onSubmit={handleSaveAnnouncement} className="flex flex-col gap-3.5 text-xs">
              <div>
                <label className="text-[#050505]  font-semibold mb-1 block">Titre :</label>
                <input
                  type="text"
                  value={formTitre}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[#050505]  font-semibold mb-1 block">Type :</label>
                  <select
                    value={formType}
                ...
                <div>
                  <label className="text-[#050505]  font-semibold mb-1 block">Public Cible :</label>
                  <select
                    value={formCible}
              ...
              <div>
                <label className="text-[#050505]  font-semibold mb-1 block">Contenu du message :</label>
                <textarea
                  rows={4}
                  value={formContenu}
