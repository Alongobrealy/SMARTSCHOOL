...
export const ReceiptModal: React.FC<ReceiptModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col my-8 transition-colors duration-200">
        ...
        <div id="printable-receipt" className="p-6 flex flex-col gap-6 text-sm bg-white dark:bg-slate-900">
          ...
          <div className="border-b border-slate-200 dark:border-slate-800 pb-4 flex items-start justify-between">
            <div>
              <h2 className="text-lg sm:text-xl font-extrabold text-indigo-900 dark:text-indigo-300 uppercase tracking-tight">
            ...
            <div className="text-right">
              <span className="inline-block bg-emerald-100 dark:bg-emerald-950/70 text-emerald-800 dark:text-emerald-300 text-xs font-bold px-2.5 py-1 rounded-md border border-emerald-200 dark:border-emerald-800">
                PAYÉ / VALIDÉ
              </span>
          ...
          <div className="grid grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-800/70 p-4 rounded-xl border border-slate-200 dark:border-slate-700 text-xs">
            <div>
              <span className="text-slate-500 dark:text-slate-400 font-medium">Bénéficiaire (Élève/Étudiant) :</span>
              <p className="font-bold text-slate-900 dark:text-white text-sm mt-0.5">{payment.studentName}</p>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 font-medium">Classe / Niveau :</span>
              <p className="font-bold text-slate-900 dark:text-white text-sm mt-0.5">{payment.classe}</p>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 font-medium">Motif de Paiement :</span>
              <p className="font-bold text-slate-900 dark:text-white mt-0.5">{payment.motif}</p>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 font-medium">Mode de Règlement :</span>
              <p className="font-bold text-slate-900 dark:text-white mt-0.5">{payment.modePaiement}</p>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 font-medium">Réf. Transaction :</span>
              <p className="font-mono text-slate-800 dark:text-slate-200 mt-0.5">{payment.referenceTransaction || 'N/A'}</p>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 font-medium">Date d'encaissement :</span>
              <p className="font-bold text-slate-900 dark:text-white mt-0.5">{payment.datePaiement}</p>
            </div>
          </div>

          {/* Amount Box */}
          <div className="bg-indigo-50/70 dark:bg-indigo-950/40 border-2 border-indigo-200 dark:border-indigo-800/70 rounded-xl p-4 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-indigo-900 dark:text-indigo-300">Montant Reçu</span>
              <p className="text-2xl font-black text-indigo-950 dark:text-white">{payment.montant.toLocaleString()} FCFA</p>
            </div>
            <div className="text-right">
              <CheckCircle2 className="w-8 h-8 text-emerald-600 dark:text-emerald-400 inline" />
            </div>
          </div>

          {/* Footer & Stamps */}
          <div className="grid grid-cols-2 gap-4 items-end pt-2 border-t border-dashed border-slate-300 dark:border-slate-700 text-xs">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300 font-mono text-[10px]">
                <QrCode className="w-12 h-12 text-slate-800 dark:text-slate-200" />
                <div>
                  <p className="font-bold text-slate-900 dark:text-white">Authenticité Certifiée</p>
                  <p className="text-slate-400 dark:text-slate-500">Scanner pour valider</p>
                </div>
              </div>
            </div>
            <div className="text-center">
              <span className="text-slate-500 dark:text-slate-400 font-semibold block text-[11px] mb-6">Le Caissier Autorisé</span>
              <span className="font-bold text-indigo-900 dark:text-indigo-300 text-xs border-b border-slate-900 dark:border-slate-300 pb-0.5">
                {payment.caissier}
              </span>
            </div>
          </div>
