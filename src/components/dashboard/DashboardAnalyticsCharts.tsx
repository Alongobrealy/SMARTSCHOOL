...
export const DashboardAnalyticsCharts: React.FC<DashboardAnalyticsChartsProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white  rounded-lg shadow-sm border border-slate-200  p-5 sm:p-6 flex flex-col justify-between gap-4 transition-colors duration-200">
        
        {/* Header with Title and Mode Toggle */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200  pb-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="p-2 rounded-xl bg-blue-50  text-blue-600  border border-slate-200 ">
                <GraduationCap className="w-5 h-5" />
              </span>
              <div>
                <h3 className="font-bold text-slate-800  text-base">Taux de Réussite par Classe (Brazzaville)</h3>
                <p className="text-xs text-slate-500 ">Performance académique & validation des moyennes (≥ 10/20)</p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-slate-50  rounded-xl p-3 border border-slate-200 ">
            <span className="text-[11px] font-semibold text-slate-500  block uppercase">Moyenne Globale</span>
            <span className="text-xl font-bold text-blue-600 ">{globalSuccessRate}%</span>
            <span className="text-[10px] text-blue-600  font-medium block">Taux d'admissibilité</span>
          </div>
          <div className="bg-slate-50  rounded-xl p-3 border border-slate-200 ">
            <span className="text-[11px] font-semibold text-slate-500  block uppercase">Classes Suivies</span>
            <span className="text-xl font-bold text-slate-800 ">{classSuccessData.length}</span>
            <span className="text-[10px] text-slate-500  font-medium block">Sections actives</span>
          </div>
          <div className="bg-slate-50  rounded-xl p-3 border border-slate-200 ">
            <span className="text-[11px] font-semibold text-slate-500  block uppercase">Objectif MEPPSA</span>
            <span className="text-xl font-bold text-blue-600 ">85%</span>
            <span className="text-[10px] text-slate-500  font-medium block">Seuil d'excellence</span>
          </div>
        </div>
                ...
                <Tooltip 
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-white text-white p-3 rounded-xl shadow-lg border border-slate-200 text-xs">
                          <p className="font-bold text-sm text-blue-600">{data.classe}</p>
                          <div className="mt-1.5 flex flex-col gap-1">
                            <p className="text-slate-500">
                              Taux de réussite : <span className="font-bold text-blue-600">{data.tauxReussite}%</span>
                            </p>
                            <p className="text-slate-500">
      ...
      <div className="bg-white  rounded-lg shadow-sm border border-slate-200  p-5 sm:p-6 flex flex-col justify-between gap-4 transition-colors duration-200">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200  pb-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-bold text-slate-800  text-base">Évolution des Paiements d'Écolage (FCFA)</h3>
                <p className="text-xs text-slate-500 ">Flux d'encaissement mensuel & canaux de paiement Congo</p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-slate-50  rounded-xl p-3 border border-slate-200 ">
            <span className="text-[11px] font-semibold text-slate-500  block uppercase">Ce Mois (Août)</span>
            <span className="text-xl font-bold text-blue-600 ">+{totalCollectedSum.toLocaleString()} FCFA</span>
            <span className="text-[10px] text-slate-500  font-medium block">Total encaissé</span>
          </div>
          <div className="bg-slate-50  rounded-xl p-3 border border-slate-200 ">
            <span className="text-[11px] font-semibold text-slate-500  block uppercase">Total Transactions</span>
            <span className="text-xl font-bold text-slate-800 ">{payments.length} Reçus</span>
            <span className="text-[10px] text-blue-600  font-medium block">100% Authentifiés</span>
          </div>
          <div className="bg-slate-50  rounded-xl p-3 border border-slate-200 ">
            <span className="text-[11px] font-semibold text-slate-500  block uppercase">Canal Principal</span>
            <span className="text-xl font-bold text-blue-600 ">MTN MoMo</span>
            <span className="text-[10px] text-blue-600  font-medium block">65% des règlements</span>
          </div>
        </div>
                ...
                <Tooltip 
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-white text-white p-3 rounded-xl shadow-lg border border-slate-200 text-xs">
                          <p className="font-bold text-sm text-blue-600">{data.nomComplet}</p>
                          <div className="mt-1.5 flex flex-col gap-1">
                            <p className="text-slate-500">
                              Encaissé : <span className="font-bold text-white">{data.encaisse.toLocaleString()} FCFA</span>
                            </p>
                            <p className="text-slate-500">
                              Prévisionnel : <span className="font-semibold text-slate-500">{data.objectif.toLocaleString()} FCFA</span>
                            </p>
