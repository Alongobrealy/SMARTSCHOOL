...
export const OverdueFeesAlertWidget: React.FC<OverdueFeesAlertWidgetProps> = ({
  ...
  if (overdueStudents.length === 0) {
    return (
      <div className="bg-emerald-50/80 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/80 rounded-2xl p-5 shadow-xs flex items-center justify-between transition-colors">
        ...
        <span className="text-xs font-mono font-bold bg-emerald-100 dark:bg-emerald-900 text-emerald-800 dark:text-emerald-200 px-3 py-1 rounded-full">
          100% Recouvert
        </span>
      </div>
    );
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border-2 border-rose-500/40 dark:border-rose-600/40 rounded-2xl shadow-md overflow-hidden transition-all duration-200">
      
      {/* Alert Header Banner */}
      <div className="bg-gradient-to-r from-rose-50 via-white to-amber-50 dark:from-rose-950/60 dark:via-slate-900 dark:to-amber-950/40 px-5 sm:px-6 py-4 border-b border-rose-200/80 dark:border-rose-900/60 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-2xl bg-rose-600 text-white flex items-center justify-center shadow-md shadow-rose-600/30 shrink-0 animate-pulse">
          ...
          <div>
            ...
            <h3 className="text-base sm:text-lg font-extrabold text-slate-900 dark:text-white mt-0.5">
              Impayés en Souffrance : <span className="text-rose-600 dark:text-rose-400 font-mono">{totalOverdueAmount.toLocaleString()} FCFA</span>
            </h3>
          </div>
        </div>
      ...
      {bulkSentSuccess && (
        <div className="bg-emerald-600 text-white text-xs font-semibold px-6 py-2.5 flex items-center justify-between animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 dark:divide-slate-800 border-b border-slate-100 dark:border-slate-800 text-xs">
        
        {/* Critical Arrears */}
        <div 
          onClick={() => setFilterSeverity(filterSeverity === 'critical' ? 'all' : 'critical')}
          className={`p-4 flex items-center justify-between cursor-pointer transition-colors ${
            filterSeverity === 'critical' ? 'bg-rose-50/70 dark:bg-rose-950/40' : 'hover:bg-slate-50 dark:hover:bg-slate-800/40'
          }`}
        >
          <div className="flex items-center gap-3">
            <div className="w-3 h-3 rounded-full bg-rose-500 animate-ping"></div>
            <div>
              <span className="font-bold text-slate-900 dark:text-white block">Retards Critiques (≥ 50k FCFA)</span>
              <span className="text-[11px] text-rose-600 dark:text-rose-400 font-semibold">Priorité 1 de relance</span>
            </div>
          </div>
          <span className="text-base font-extrabold text-rose-600 dark:text-rose-400 font-mono bg-rose-100 dark:bg-rose-900/60 px-2.5 py-0.5 rounded-lg">
            {criticalCount}
          </span>
        </div>
        ...
        <div 
          ...
          className={`p-4 flex items-center justify-between cursor-pointer transition-colors ${
            filterSeverity === 'moderate' ? 'bg-amber-50/70 dark:bg-amber-950/40' : 'hover:bg-slate-50 dark:hover:bg-slate-800/40'
          }`}
        >
          <div className="flex items-center gap-3">
            <div className="w-3 h-3 rounded-full bg-amber-500"></div>
            <div>
              <span className="font-bold text-slate-900 dark:text-white block">Soldes Partiels (&lt; 50k FCFA)</span>
              <span className="text-[11px] text-amber-600 dark:text-amber-400 font-semibold">Relance de routine</span>
            </div>
          </div>
          <span className="text-base font-extrabold text-amber-600 dark:text-amber-400 font-mono bg-amber-100 dark:bg-amber-900/60 px-2.5 py-0.5 rounded-lg">
            {moderateCount}
          </span>
        </div>
        ...
        <div className="p-4 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-bold text-slate-900 dark:text-white block">Élèves Totalement à Jour</span>
              <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold">Situation saine</span>
            </div>
          </div>
          <span className="text-base font-extrabold text-emerald-600 dark:text-emerald-400 font-mono bg-emerald-100 dark:bg-emerald-900/60 px-2.5 py-0.5 rounded-lg">
      ...
      {isExpanded && (
        <div className="p-5 sm:p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative flex-1 sm:w-64">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-xl pl-8 pr-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-rose-500"
                />
              </div>
              ...
              <select
                ...
                className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-xl px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-rose-500 cursor-pointer"
              >
            ...
            <div className="text-slate-500 dark:text-slate-400 text-xs font-semibold text-right">
                    ...
                    return (
                      <tr 
                        ...
                        className={`transition-colors ${
                          st.isCritical 
                            ? 'hover:bg-rose-50/50 dark:hover:bg-rose-950/20 bg-rose-50/20 dark:bg-rose-950/10' 
                      ...
                      >
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2.5">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${
                              st.isCritical ? 'bg-rose-100 dark:bg-rose-900/60 text-rose-700 dark:text-rose-300' : 'bg-amber-100 dark:bg-amber-900/60 text-amber-700 dark:text-amber-300'
                            }`}>
                        ...
                        <td className="py-3 px-4">
                          <span className="font-semibold text-slate-800 dark:text-slate-200 block">{st.nomParent}</span>
                          <span className="text-[11px] text-indigo-600 dark:text-indigo-400 font-mono flex items-center gap-1">
                        ...
                        <td className="py-3 px-4 text-right font-mono font-bold text-emerald-600 dark:text-emerald-400">
                          ...
                          <span className="block text-[10px] text-slate-400 font-normal">({st.pourcentagePaye}%)</span>
                        </td>

                        <td className="py-3 px-4 text-right font-mono font-extrabold text-rose-600 dark:text-rose-400 text-sm">
                          -{st.resteAPayer.toLocaleString()} FCFA
                        </td>

                        <td className="py-3 px-4 text-center">
                          {st.isCritical ? (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-rose-100 dark:bg-rose-900/60 text-rose-700 dark:text-rose-300 border border-rose-300 dark:border-rose-800">
                              <span className="w-1.5 h-1.5 rounded-full bg-rose-500"></span>
                              Critique
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-100 dark:bg-amber-900/60 text-amber-700 dark:text-amber-300 border border-amber-300 dark:border-amber-800">
                        ...
                        <td className="py-3 px-4 text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            <a
                              ...
                              className="p-1.5 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/60 dark:hover:bg-emerald-900 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 rounded-lg transition-colors cursor-pointer"
                            ...
                            {onOpenPaymentModal && (
                              <button
                                ...
                                className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-[11px] font-bold shadow-2xs transition-colors cursor-pointer flex items-center gap-1"
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 p-3.5 rounded-xl border border-slate-200/60 dark:border-slate-700/60 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-slate-600 dark:text-slate-400">
            <div className="flex items-center gap-2">
