...
# From: `  return (` to `<main className="flex-1 flex flex-col min-w-0">`

# Find the start
start_idx = text.find('  return (')

# Find the end of the sidebar / start of main
main_idx = text.find('<main className="flex-1 flex flex-col min-w-0">')

if start_idx != -1 and main_idx != -1:
    # Extract existing logo, nav links, and profile out of the current layout? 
    # Or just write them from scratch so they are perfect?
    # Let's write the whole layout skeleton up to the `<main>` and the new header.
        ...
        <div className="p-4 border-t border-[#1E293B] bg-slate-900/50">
          ...
          <div className="flex gap-2 mt-3">
             <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-2 py-2 rounded-lg text-xs font-bold bg-slate-800 hover:bg-slate-700 text-white transition-colors"
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-2 py-2 rounded-lg text-xs font-bold bg-red-500/10 hover:bg-red-500/20 text-red-500 transition-colors"
                title="Déconnexion"
              >
...
"""
    # Replace the block
    # Note that <main className="flex-1 flex flex-col min-w-0"> has some content after it we need to preserve or remove.
    # We want to replace from `start_idx` to `div className="flex-1 p-4 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto">`
    # Let's find `<div className="flex-1 p-4 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto">`
    content_start_idx = text.find('<div className="flex-1 p-4 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto">')
    if content_start_idx != -1:
        text = text[:start_idx] + new_sidebar_and_header + text[content_start_idx + len('<div className="flex-1 p-4 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto">'):]
    
        with open('src/components/DashboardLayout.tsx', 'w') as f:
            f.write(text)
        print("DashboardLayout successfully updated!")
    else:
        print("Could not find content_start_idx")
